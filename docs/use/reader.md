...
- 翻页/工具条：由“点击区域”决定（默认 3 段：左侧上一页 / 中间切换进度工具条 / 右侧下一页；也可横向/竖向拆分为更多区域）。
- 收起进度工具条时，会自动关闭书签、文件信息等展开面板，避免界面残留与遮挡。
- 可视化配置：在阅读器工具条“展开模式”下，点击“点击区域”按钮进入配置界面；也可在“设置” -> “阅读器设置” -> “点击区域”中配置。
- 调整范围：在配置界面拖动分割线，即可改变各个区域大小；也可通过“横向/竖向拆分、横向/竖向合并”来增减区域数量。
...
- 点击区域：可将画面划分为多行多列区域并绑定动作（上一页、下一页、切换/展开/收起进度工具条等），拖动分割线调整大小，并支持横向/竖向拆分与合并。

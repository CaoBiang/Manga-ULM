...
const handleRemoveFromWishlist = async (fileId) => {
    if (!confirm('Are you sure you want to remove this manga from your wishlist?')) {
        return;
    }
    try {
...
<template>
  <div class="p-6 bg-white rounded-lg shadow">
    <h2 class="text-xl font-semibold text-gray-700 mb-4">My Wishlist</h2>
    ...
    <div v-else-if="wishlistItems.length > 0">
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        <div v-for="manga in wishlistItems" :key="manga.id" class="relative group/wishlist">
            ...
            <button @click="handleRemoveFromWishlist(manga.id)" class="absolute top-2 right-2 z-20 p-1.5 bg-red-600 rounded-full text-white opacity-0 group-hover/wishlist:opacity-100 transition-opacity" title="Remove from Wishlist">
